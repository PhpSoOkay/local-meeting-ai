...
def _ensure_init():
    ...
    # Определяем BASE_DIR напрямую, без относительных импортов
    # Это важно, так как process_meeting.py запускается как subprocess
    base = Path.home() / "Yandex.Disk/carrier/meeting-ai"
    LOG_DIR = base / "logs"
